...
class HotNewsCollector:
    ...
    async def collect_news(self, sources: Optional[List[str]] = None) -> Dict:
        """
        收集热点新闻
        ...
        try:
            ...
            # 打印统计信息
            logger.info(f"\n总新闻源: {len(results)}")
            logger.info(f"成功源数: {successful_sources}")
            logger.info(f"总新闻数: {total_news}")
            logger.info(f"已处理数: {len(news_list)}")
...
# 全局实例
hot_news_collector = HotNewsCollector()
