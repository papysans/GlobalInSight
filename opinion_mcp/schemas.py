...
class AnalysisCards(BaseModel):
    ...
    platform_radar: Optional[str] = Field(default=None, description="平台雷达图 URL")
...
class AnalysisResult(BaseModel):
    ...
    subtitle: str = Field(default="", description="副标题")
    platforms_analyzed: List[str] = Field(default_factory=list, description="已分析平台列表")
    ...
    platform_stats: Dict[str, int] = Field(default_factory=dict, description="各平台数据量统计")
...
class SettingsResponse(BaseModel):
    ...
    available_platforms: List[PlatformInfo] = Field(default_factory=list, description="可用平台列表")
...
class AnalysisResultResponse(BaseModel):
    ...
    platforms_analyzed: List[str] = Field(default_factory=list, description="已分析平台")
