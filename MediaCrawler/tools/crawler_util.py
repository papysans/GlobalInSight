...
def show_qrcode(qr_code) -> None:  # type: ignore
    ...
    # Fix for Windows compatibility: UnixViewer may not exist or have options on Windows
    try:
        if hasattr(ImageShow, 'UnixViewer') and hasattr(ImageShow.UnixViewer, 'options'):
            if "save_all" in ImageShow.UnixViewer.options:
                del ImageShow.UnixViewer.options["save_all"]
    except (AttributeError, KeyError):
        pass  # Ignore on Windows or if structure is different
    new_image.show()
