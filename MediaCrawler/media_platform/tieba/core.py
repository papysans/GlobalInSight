...
class TieBaCrawler(AbstractCrawler):
    ...
    async def search(self) -> None:
        ...
        # Wait for potential second captcha before starting search
        # Tieba may trigger another verification when accessing search API
        search_captcha_wait = 15  # 15 seconds for second captcha
        utils.logger.info(f"[BaiduTieBaCrawler.search] 等待 {search_captcha_wait} 秒以便完成第二次人机验证（如有）...")
        await asyncio.sleep(search_captcha_wait)
        utils.logger.info(f"[BaiduTieBaCrawler.search] 第二次验证等待结束，开始执行搜索...")
    ...
    async def _navigate_to_tieba_via_baidu(self):
        ...
        try:
            ...
            # Step 6: Extra wait time for first captcha/verification after entering Tieba
            # Tieba often triggers captcha when entering from Baidu homepage
            captcha_wait_time = 15  # 15 seconds for user to complete first captcha
            utils.logger.info(f"[TieBaCrawler] Step 6: 等待 {captcha_wait_time} 秒以便完成第一次人机验证（如有滑块或点选验证）...")
            await asyncio.sleep(captcha_wait_time)
            utils.logger.info(f"[TieBaCrawler] 第一次验证等待结束...")
