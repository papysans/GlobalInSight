    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if config.ENABLE_CDP_MODE:
                ...
                await self.browser_context.add_init_script(path=os.path.join(_libs_path, "stealth.min.js"))
    ...
    async def search(self):
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * weibo_limit_count <= config.CRAWLER_MAX_NOTES_COUNT:
                ...
                # 添加调试日志
                if not search_res:
                    utils.logger.warning(f"[WeiboCrawler.search] 搜索结果为空: search_res is None or empty")
                else:
                    cards = search_res.get("cards")
                    if not cards:
                        utils.logger.warning(f"[WeiboCrawler.search] 搜索结果中没有 cards 字段，可能的原因：1) 关键词被限制 2) 账号未登录 3) 账号被风控。完整响应: {str(search_res)[:500]}")
                    else:
                        utils.logger.info(f"[WeiboCrawler.search] 搜索返回 {len(cards)} 个卡片")
                        # 检查卡片类型分布
                        card_types = {}
                        for card in cards:
                            card_type = card.get("card_type")
                            card_types[card_type] = card_types.get(card_type, 0) + 1
                        utils.logger.info(f"[WeiboCrawler.search] 卡片类型分布: {card_types}")
                
                note_id_list: List[str] = []
                note_list = filter_search_result_card(search_res.get("cards") if search_res else [])
                utils.logger.info(f"[WeiboCrawler.search] 过滤后得到 {len(note_list)} 个有效笔记 (card_type=9)")
